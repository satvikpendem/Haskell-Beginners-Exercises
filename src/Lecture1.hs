...
-- DON'T FORGET TO SPECIFY THE TYPE IN HERE
lastDigit :: Int -> Int
lastDigit n = mod (if n < 0 then -n else n) 10
...
subString start end str
    | end < 0 = ""
    | end < start = ""
    | otherwise = take (end - newStart + 1) (drop newStart str)
  where
    newStart = max start 0
...
-}
strSum :: String -> Int
strSum str = foldr (\x acc -> acc + read x) 0 (words str)
...
lowerAndGreater n list =
    let greater :: Int = foldr (\x acc -> if n > x then acc + 1 else acc) 0 list
     in let lesser :: Int = foldr (\x acc -> if n < x then acc + 1 else acc) 0 list
         in printf "%d is greater than %d elements and lower than %d elements" n greater lesser
